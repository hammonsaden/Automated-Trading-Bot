...
class TraderBot:
    ...
    def update_portval(self, conn, finnhub):
        ...
        for val in rows:
            amount_owned = val[1]
            if val[0] == "MONEY":
                stock_val = 1.00 * amount_owned  
                stock_vals.append(stock_val)
            else:
                data = finnhub.quote(str(val[0]))  
                data_list = list(data.values())
            
                # Ensure the data contains valid price information
                try:
                    stock_price = float(data_list[0])
                    stock_val = stock_price * amount_owned
                    stock_vals.append(stock_val)
                    print(f"STOCK: {val[0]} VALUE EQUATION: {stock_price} * {amount_owned} = {stock_val}")
                except (ValueError, IndexError) as e:
                    print(f"Error fetching or parsing data for {val[0]}: {e}")
        ...
        threading.Timer(120, lambda: self.update_portval(conn, finnhub)).start() # 2 minutes
    ...
    def update_watchlist(self, conn, finnhub, watchlist_bool):
        ...
        for val in rows:
            if val[0] == "MONEY":
                ...
                cur_datetime = datetime.now()
                form_dt = cur_datetime.strftime("%H:%M:%S")
                approx_val = amount_owned[0] * 1
                cur.execute("UPDATE watchlist SET last_price=1, amount_owned = ?, approx_value = ? WHERE ticker = ?", (round(amount_owned[0], 2), round(approx_val, 2), val[0]))
                conn.commit()
            else:
                data = finnhub.quote(str(val[0]))
                data_list = list(data.values())
                ...
                conn.commit()
                if amount_owned:
                    approx_val = round(float(amount_owned[0]) * float(data_list[0]), 2)
                    # Updating Watchlist
                    cur.execute("UPDATE watchlist SET last_price = ?, amount_owned = ?, approx_value = ? WHERE ticker = ?", (data_list[0], amount_owned[0], approx_val, val[0]))
                    conn.commit()
                else:
    ...
    def make_buy(self, conn, ticker, avg_buy_price, amount_to_buy):
        ...
        if trade_count < 5:
            small_trade_amount = min(amount_to_buy, 5)
            total_cost = avg_buy_price * small_trade_amount
            print(f"Small trade: Adjusting amount to {small_trade_amount}")
        else:
        ...
        if available_money >= total_cost:
            ...
            cur.execute("INSERT INTO trades (datetime, ticker, price, amount, value) VALUES (datetime('now'), ?, ?, ?, ?)",
    ...
    def make_sale(self, conn, ticker, avg_sell_price, amount_to_sell):
        ...
        if available_amount >= amount_to_sell:
            ...
            cur.execute(
                ...
                (ticker, avg_sell_price, -amount_to_sell, sale_value) 
            )
    ...
    def eval_stock(self, conn):
        '''Evaluate all prices and data to make decisions to buy or sell.'''
        self.trade_allowed = True

        if self.trade_allowed:
            # Dictionaries to compare prices
            average_prices = {}
            avg_trade_prices = {}
            cur_prices = {}
            ...
            # Fetch trade count
            cur.execute("SELECT COUNT(*) FROM trades")
            self.trade_count = cur.fetchone()[0]
            ...
            if watchlist:
                ...
                for val in price_history:
                    ...
                    if ticker != "MONEY" and ticker in ticker_data:
                ...
                # Fetch trade history
                cur.execute("SELECT ticker, price FROM trades WHERE ticker IN ({})".format(','.join(["?"] * len(watchlist))), watchlist)
                trade_history = cur.fetchall()

                trade_data = {ticker: [] for ticker in watchlist}
                ...
                for ticker, prices in trade_data.items():
                    if prices:
                        avg_trade_prices[ticker] = sum(prices) / len(prices)
                ...
                for ticker in watchlist:
                    ...
                    cur_prices[ticker] = float(cur_price)

                # Main stock evaluation
                for stock in watchlist:
                    if stock in average_prices and stock in cur_prices:
                        current_price = cur_prices[stock]
                        historical_avg = average_prices[stock]
                        trade_avg = avg_trade_prices.get(stock, historical_avg)  # Default to historical_avg if no trade history

                        # Starter trade logic
                        if self.trade_count < 5:
                            amount_to_buy = 5  
                            print(f"Starter trade for {stock} at price {current_price}")
                            self.make_buy(conn, stock, current_price, amount_to_buy)
                            self.trade_count += 1
                            continue

                        # Buy decision
                        if current_price < historical_avg and current_price < trade_avg:
                            amount_to_buy = 10  # Example: Standard trade amount
                            print(f"Time to buy {stock} at price {current_price}")
                            self.make_buy(conn, stock, current_price, amount_to_buy)

                        # Sell decision
                        cur.execute("SELECT amount FROM portfolio_stocks WHERE ticker = ?", (stock,))
                        amount_owned = cur.fetchone()

                        if amount_owned and amount_owned[0] > 0 and current_price > historical_avg and current_price > trade_avg:
                            print(f"Time to sell {stock} at price {current_price}")
                            self.make_sale(conn, stock, current_price)  # Assuming make_sale method is defined
...
class TraderApp:
    ...
    def main_GUI(self):
        ...
        self.value_text = ctk.CTkLabel(mainFrame, text="Portfolio Value $ " + str(portfolio_val), font=("Helvetica", 18))
        self.value_text.grid(row=0, column=1, sticky="e", padx=10)

        # Fetch money available
        cur.execute("SELECT amount FROM portfolio_stocks WHERE ticker = ?", ("MONEY",))
        money_val = cur.fetchone()

        # Display money available label
        self.money_text = ctk.CTkLabel(mainFrame, text="Money $ " + str(money_val[0]), font=("Helvetica", 18))
        self.money_text.grid(row=0, column=1, sticky="w", padx=10)
        ...
        # Redraw portfolio value and available money
        self.redraw_portfolio_and_money()
    ...
    def redraw_portfolio_and_money(self):
        # Fetch and update portfolio value
        cur = self.portfolioconn.cursor()
        cur.execute("SELECT value FROM portfolio ORDER BY datetime DESC LIMIT 1")
        last_val = cur.fetchone()
        ...
        self.value_text.configure(text="Portfolio Value $ " + str(portfolio_val))

        # Fetch and update money available
        cur.execute("SELECT amount FROM portfolio_stocks WHERE ticker = ?", ("MONEY",))
        money_val = cur.fetchone()

        # Update money available label
        self.money_text.configure(text="Money $ " + str(money_val[0]))

        print("PORT AND MONEY UPDATED!")
        threading.Timer(121, lambda: self.redraw_portfolio_and_money()).start() # 2 minutes 1 second

    def check_watchlist_update(self):
        if self.watchlist_bool[0]:  
            self.redraw_watchlist()  
